...
def convnet_model_():
    vgg_model = InceptionResNetV2(weights=None, include_top=False, input_shape = (224,224,3))
    x = vgg_model.output
    x = GeMPooling2D()(x)
...
def generator_three_img_sampling(gen,dir1,target_size=(224,224), batch_size=64, class_mode="categorical"):
    ...
    num_pos = 5
    num_neg = 1
    num_neg_class = int((batch_size - num_pos)/num_neg)

    pos_genX_list = []
    neg_genX_list = []

    for train_class in train_class_list:
        pos_genX = gen.flow_from_directory(dir1, classes=[train_class],target_size = target_size, batch_size=num_pos, 
            class_mode=class_mode, shuffle = True, seed=7)
        neg_genX = gen.flow_from_directory(dir1, classes=[train_class],target_size = target_size, batch_size=num_neg, 
            class_mode=class_mode, shuffle = True, seed=7)

        pos_genX_list.append(pos_genX)
        neg_genX_list.append(neg_genX)

    while True:

        # choose positive class
        pos_class = np.random.randint(0,num_class)
        total_data = pos_genX_list[pos_class].next()
        img_data = preprocess_input(total_data[0])
        label_data = np.array([pos_class for i in range(len(total_data[0]))])

        # choose negative classes
        neg_list = np.random.randint(0,num_class, size = num_neg_class)
        for n in neg_list:
            tmp_total = neg_genX_list[n].next()
            img_data = np.concatenate((img_data, preprocess_input(tmp_total[0])))
            label_data = np.concatenate((label_data, np.array([n for i in range(len(tmp_total[0]))])))
...
if __name__ == '__main__':
    ...
    if config.mode == 'train':
        ...
        '''
        nsml.load(checkpoint='0', session='team_52/ir_ph2/422')
        nsml.save('saved')
        exit()
        '''
        ...
        nsml.load(checkpoint='1', session='team_52/ir_ph2/422')
        train_dataset_path = DATASET_PATH + '/train/train_data'
        train_class_list = os.listdir(train_dataset_path)
